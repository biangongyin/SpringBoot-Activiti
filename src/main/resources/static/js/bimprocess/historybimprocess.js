    $(document).ready(function(){
	    ...
	    var grid=$("#grid-data").bootgrid({
		    ...
		    ajax:true,
		    url:"getFinishBimProcess",
		    formatters: {
		    "process_instance_id": function(column, row){
		    	return row.bimLeaveapply.process_instance_id;
		    },
		    "user_id":function(column, row){
		    	return row.bimLeaveapply.user_id;
		    },
		    "leave_type":function(column, row){
		    	return row.bimLeaveapply.leave_type;
		    },
		    "apply_time":function(column, row){
		    	return row.bimLeaveapply.apply_time;
		    },
		    "commands": function(column, row)
		    {
		            return "<button class=\"btn btn-xs btn-default ajax-link command-run1\" data-row-id=\"" + row.bimLeaveapply.process_instance_id + "\">查看详情</button>";
		    }
	    	}
	    	    ...
	    	    grid.find(".command-run1").on("click", function(e)
	    	    {
	    	    	...
	    	    	$.post("bimProcessinfo",{"instanceid":process_instance_id},function(data){
